...
- llm-client / config - **breaking**: the tool-result seam split into two independently-selected seams, and core ships no implementation of either. `WireFormat` owns the model-facing *markup shape* of harness wrappers (tool results, attached-file includes, system notices): `modelToolFormat`/`--model-tool-format` became `modelWireFormat`/`--model-wire-format` (default still `"xml"`, but the shape -- and the `file-include`/`system-notice` marker protection -- now come from the autoloaded `wire-format-xml` extension). `RoleMapping` owns where the internal `harness` role rides on the wire: the provider/model entry `wireFormat: "system-first" | "developer"` moved to `roleMapping` with the same two ids, registered by the new autoloaded `role-mapping-default`. Migration: `wireFormat` on a provider/model entry now names a WireFormat, so a stale `wireFormat: "developer"` config fails on the first request with `Unknown wire format "developer"` -- rename the key to `roleMapping`. Tool results now travel as structured parts (tool/status/meta/error/hint/output) through context, session logs, hooks, and UI events instead of pre-rendered text: nothing below the wire holds markup, the wire serializer is the only render point (fields mangle before the format sees them, so a format physically cannot emit an unescaped payload), and every display surface composes its own text. Wire breaks for extension authors: the `turn:end` hook payload's `result` is `content`, same for the TOOL_RESULT output event; `ToolResult.outputTag` removed; `EXTENSION_PROVIDES.TOOL_FORMATS` replaced by `WIRE_FORMATS` + `ROLE_MAPPINGS`. A seam name configured but registered by nothing throws `Unknown wire format/role mapping "<name>"` (config) at request build; a name simply unset throws at the wire only when a message actually needs shaping -- never a silent fallback
