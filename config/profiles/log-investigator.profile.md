...
- `./target/debug/oa-agent review --json` — lists recent sessions as JSON array
- `./target/debug/oa-agent review --session-id <id> --json` — outputs raw JSONL entries for a session
- `./target/debug/oa-agent review --session-id <id> --json --tool-index` — outputs JSON tool index
...
Run `./target/debug/oa-agent review --json` to list recent sessions. Parse the JSON array. Pick 2-3 sessions that are long enough to analyze (more than ~20 entries is a good minimum).
...
```bash
./target/debug/oa-agent review --session-id <id> --json --tool-index
```
...
```bash
./target/debug/oa-agent review --session-id <id> --json | grep -B2 -A2 '"function":{"name":"<suspicious_tool>"'
```
