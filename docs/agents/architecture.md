...
- `client.ts` — `LlmClient` with streaming, cancellation, retry support. Keeps only transport concerns (retry, timeout, cancellation) and delegates backend-specific work to the resolved LlmProtocol. For streaming, the retry unit is the request **plus full stream consumption** (a half-read SSE body cannot be resumed): a mid-stream network failure re-issues the whole request and the client emits a `reset` StreamEvent so consumers discard the failed attempt's partial output (see `StreamEvent` in `client.ts` and the `reset` case in `StreamProcessor.process`). The per-attempt timeout stays on `hotdogFetch`'s independent `AbortSignal.timeout`; the shared abort controller is reserved for user cancellation, which never retries.
...
- `serialize.ts` — wire serialization of internal `Message`s to the OpenAI-compatible wire shape, per-model chat-template format (system-first vs developer), including provenance-based marker mangling at the wire boundary and rendering of semantic wrapper parts (file-include, system-notice) via `context/wrappers.ts`
- `retry.ts` — retry classification and wait primitives: `shouldRetryLlmError(e, attempt, maxRetries)` (http/timeout always transient; api only on retryable status; cancelled never) and `retryDelay(ms, signal)` (aborts early on user cancellation, detaches its listener when done). `LlmClient.chatStreamCancellable` drives its own backoff loop with these. `maxRetries` counts retries *after* the initial attempt (total attempts = `1 + maxRetries`); `0`/negative means one attempt, no retries. `ToolExecutor` uses the same convention.
...
### Spoof Neutralizer (`src/utils/spoof.ts`)
Protects the HUMAN's eyes where the mangler protects the model's. `neutralizeSpoofing(text)` replaces every spoofing code point (bidi controls U+202A..U+202E, isolates U+2066..U+2069, zero-widths, BOM, C0/C1 controls incl. ESC -- terminal control sequences are spoofing vectors where Unicode isn't) with a visible `[U+XXXX]` token and counts it; never silently stripped. `spoofSafe(text)` is the text-only convenience. INVARIANT: neutralize at RENDER, never at storage -- the session log keeps original bytes (audit), every human-facing surface neutralizes. Applied at one choke per surface: `CliOutputSink` tool-call/question display, `ui-interactive-cli` collectAnswers prompts, `user-gate` approval prompt composition, webui `sanitize()`. Idempotent (tokens are pure ASCII), so stacked choke points are harmless.
...
- `workspace.ts` — multi-root `Workspace` class: escape-safe path resolution against `workspace.paths` roots (rejects traversal and symlink escapes), plus a path-component denylist driven by the `workspace.deny` config key (default in core.config.json: `.ssh`, `.config`, `.git`, `.aws`, `.azure`, `.docker`, `.gnupg`, `.kube`, `.*profile`, `.*rc`, `*.local*`, `.env*` with gitignore-style `!` negations; a root sitting at/below a denied level is exempt from that rule; `DEFAULT_DENY_PATTERNS` is the standalone-construction fallback), used by all file tools. `expandWorkspacePaths()` resolves raw config entries (tilde, globs via `Bun.Glob`, existence checks) into concrete absolute roots at config time
- `objects.ts` — `deepMerge(...sources)`
- `render.ts` — Template engine with `{{ vars }}`, `{% if %}`, `{% for %}`, filters
- `jsx/` — Zero-dependency JSX runtime (element factory, `renderToString` SSR, keyed-diff `mount` for the browser; tsconfig `jsxImportSource: "@utils/jsx"`). See `docs/agents/jsx.md`
- `reactive-state.ts` — `reactiveState<T>(initial)` tiny reactive atom returning a getter/setter callable with `.effect(fn)`; pairs with `jsx` `mount()` for UI updates
