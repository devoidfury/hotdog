...
- `ToolResult` — structured result with `output`, `error`, `hint`, `metadata`, `success`, `toDisplay()`, `toApiContent()` (returns a `ToolResultPart`, never markup)
...
Both render identically for the model: the WireFormat seam emits the hint as a structured `hint` element after the error (the element name is in `xmlWireFormat.markers`, so the wire mangler protects it). Human-facing `toDisplay()` shows it as a `HINT:` line.
...
| `string_length` | `string-length` | Measures a string in utf16, codepoints, bytes (UTF-8), and graphemes | `string` |
