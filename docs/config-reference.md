...
- **Type:** `string[]`
- **Default:** `[".ssh", ".config", ".git", ".aws", ".azure", ".docker", ".gnupg", ".kube", ".*profile", ".*rc", "*.local*", ".env*", "!.env.example"]`
...
| `pasteMarkerMinChars` | `number` | `400` | Minimum length (characters) for a single-line pasted text to be shown as a `[Paste #N]` marker; shorter single-line pastes (with any surrounding newlines trimmed) are inserted into the line as-is. Set to `0` to always use the marker. |
