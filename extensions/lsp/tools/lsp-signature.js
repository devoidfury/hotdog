...
export const LspSignatureTool = definePositionTool({
  name: 'lsp-signature',
  description:
    'Get signature help (function parameter hints) at a given position. Returns active signature, parameter info, and documentation.',
  lspMethod: 'textDocument/signatureHelp',
  requiredCapability: 'signatureHelpProvider',
  successResponse: 'No signature help available at this position.',
  formatResult: (self, result, args, resolvedPath, languageId, lspLine) => {
    ...
    lines.push(
      `Active signature: ${result.activeSignature ?? 0}/${result.signatures.length - 1}`,
    );
    lines.push(
      `Active parameter: ${result.activeParameter ?? 0}/${(activeSig?.parameters?.length ?? 1) - 1}`,
    );
    lines.push('');
    ...
    for (let i = 0; i < result.signatures.length; i++) {
      ...
      if (sig.parameters) {
        for (let j = 0; j < sig.parameters.length; j++) {
          ...
          lines.push(
            `${pPrefix}${pText}${type}${docText ? ` — ${docText.slice(0, 100)}` : ''}`,
          );
        }
      }
      lines.push('');
