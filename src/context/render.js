...
function tokenize(template) {
  ...
  while (i < template.length) {
    if (template[i] === "{" && template[i + 1] === "{") {
      ...
      const expr = stripRight ? inner.slice(0, inner.lastIndexOf("-")).trim() : inner.trim();
      ...
      const cleanTag = stripRight ? tag.slice(0, tag.lastIndexOf("-")).trim() : tag;
      tokens.push({ type: "tag", value: cleanTag, stripLeft: stripLeft || prevStripRight, stripRight });
      i = end + 2;
      plainStart = i;
      prevStripRight = stripRight;
    }
...
function pushText(tokens, template, start, end, stripLeft) {
  ...
  if (text.length > 0 || stripLeft) tokens.push({ type: "text", value: text, stripLeft });
}
...
function walkTokens(tokens, context) {
  ...
  while (idx < tokens.length) {
    ...
    if (tok.type === "text") { output += tok.value; idx++; }
    else if (tok.type === "print") { output += evalPrint(tok.value, context); idx++; }
    else if (tok.type === "tag") {
      if (tok.value.startsWith("if ")) { output += walkIf(tokens, idx, context); idx = skipPast(tokens, idx, "if", "endif"); }
      else if (tok.value.startsWith("for ")) { output += walkFor(tokens, idx, context); idx = skipPast(tokens, idx, "for", "endfor"); }
      else { idx++; }
    }
  }
  return output;
...
function skipPast(tokens, idx, openTag, closeTag) {
  ...
  while (j < tokens.length) {
    if (tokens[j].type !== "tag") { j++; continue; }
...
function walkIf(tokens, idx, context) {
  ...
  const { bodyStart, elseIdx, bodyEnd } = findBlock(tokens, idx, "if", "endif", "else");
...
function walkFor(tokens, idx, context) {
  ...
  if (Array.isArray(items)) {
    for (const item of items) {
      output += walkTokens(tokens.slice(bodyStart, bodyEnd), { ...context, [varName]: item });
    }
  }
  if (tok.stripRight) output = output.replace(/^\s+/, "");
  return output;
...
function findBlock(tokens, fromIdx, openTag, closeTag, elseTag) {
  let depth = 1, bodyStart = fromIdx + 1, elseIdx = -1, bodyEnd = tokens.length;
  for (let j = bodyStart; j < tokens.length; j++) {
    if (tokens[j].type !== "tag") continue;
    if (tokens[j].value.match(new RegExp(`^${openTag}\s`))) depth++;
    else if (tokens[j].value === closeTag) { depth--; if (depth === 0) { bodyEnd = j; break; } }
    else if (elseTag && tokens[j].value === elseTag && depth === 1) elseIdx = j;
  }
...
function applyFilter(value, filterSpec) {
  ...
  const argsStr = filterSpec.slice(parenIdx + 1, filterSpec.lastIndexOf(")")).trim();
  const namedArgs = parseNamedArgs(argsStr);
  if (name === "default") {
    const fallback = namedArgs.value !== undefined ? namedArgs.value : argsStr.replace(/^['"']/, "").replace(/['"']$/, "");
...
function applySimpleFilter(value, name) {
  if (name === "default") return value === "" || value == null ? "" : String(value);
  if (name === "length") return String((value ?? "").length);
  if (name === "trim") return String(value).trim();
  if (name === "exec") {
    const { execSync } = require("child_process");
    const cmd = typeof value === "string" ? value : String(value);
    try { return execSync(cmd, { encoding: "utf-8" }).trimEnd(); }
    catch { return ""; }
  }
  return String(value);
}
...
function resolveExpr(expr, context) {
  expr = expr.trim();
  if (expr.startsWith("not ")) return !resolveExpr(expr.slice(4).trim(), context);
...
function resolveValue(expr, context) {
  expr = expr.trim();
  if (expr.startsWith("not ")) return !resolveValue(expr.slice(4).trim(), context);
  let value = context;
  for (const key of expr.split(".")) {
