...
export class WriteTool {
  ...
  callDisplay(input: string | Record<string, unknown> | null): string {
    ...
    const search_re = args.search_re as string;
  ...
  async execute(input: string | Record<string, unknown> | null, ctx: ToolExecutionContext): Promise<ToolResult> {
    ...
    const args: WriteArgs = {
      ...
      search: (rawArgs.search as string) || null,
      search_re: (rawArgs.search_re as string) || null,
      start_at: typeof rawArgs.start_at === "number" ? rawArgs.start_at : null,
      end_at: typeof rawArgs.end_at === "number" ? rawArgs.end_at : null,
      replace_all: rawArgs.replace_all as boolean || false,
    };
    ...
    try {
      ...
      return ToolResult.err(`Edit failed: ${(e as Error).message}`);
    }
 ...
 */
function applyEdit(source: string, mode: string, content: string, search: string | null, search_re: string | null, start_at: number | null, end_at: number | null): string {
  switch (mode) {
    case "overwrite":
      return content;
    case "insert_before":
...
function applyInsert(source: string, startLine: number, content: string): string {
...
function applyRegexReplace(source: string, pattern: string, replacement: string): string {
  const re = new RegExp(pattern, "g");
  return source.replace(re, replacement);
}

function applyRangeReplace(source: string, startLine: number, endLine: number, content: string): string {
  const lines = source.split("\n");
