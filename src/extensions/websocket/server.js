...
export function createWsServer(core, options = {}) {
  ...
  const buildAgent = customBuildAgent || (async (agentConfig) => {
    ...
    const llmClient = new LlmClient({
      ...
      chatTimeoutSecs: core.resolved?.chatTimeout || 300,
    ...
    const agent = new Agent({
      ...
      llmClient,
      model: agentConfig.model || core.resolved?.model || "qwen3.5-0.8b",
      maxIterations: core.config?.maxIterations || 1000,
      maxTokens: core.config?.maxTokens || 32000,
