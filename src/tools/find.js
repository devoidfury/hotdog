  ...
  toToolDef() {
    return toolDef(
      ...
      'Find files and directories matching a glob pattern. Use this tool when you need to find files by name patterns.',
      {
        ...
        properties: {
          pattern: param('string', 'Glob pattern to match files against (e.g. "*.rs", "**/test*", "/etc/**/*.conf")'),
          file_type: param('string', 'Filter by file type: "f" for files, "d" for directories.', { enum: ['f', 'd'] }),
          max_results: param('integer', `Maximum number of results to return. Defaults to ${DEFAULT_FIND_MAX_RESULTS}.`, { minimum: 1, maximum: 10000 }),
          path: param('string', 'Path to search in. Defaults to current directory.'),
        },
        required: ['pattern'],
      }
    );
...
function parseArgs(input) {
  ...
  const max_results = typeof json.max_results === 'number' && json.max_results >= 0
    ? json.max_results
