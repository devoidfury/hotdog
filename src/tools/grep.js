...
const SKIP_DIRS = new Set([
  'node_modules', '.git', '.cargo', 'target', 'vendor', '.venv', '__pycache__',
]);
...
const TYPE_EXTENSIONS = {
  ...
  java: ['java'],
  c: ['c', 'h'],
  cpp: ['cpp', 'cxx', 'cc', 'hpp', 'hxx'], cxx: ['cpp', 'cxx', 'cc', 'hpp', 'hxx'], cc: ['cpp', 'cxx', 'cc', 'hpp', 'hxx'],
  rb: ['rb'], ruby: ['rb'],
  sh: ['sh', 'bash'], bash: ['sh', 'bash'], shell: ['sh', 'bash'],
  ...
  css: ['css', 'scss', 'less'], scss: ['css', 'scss', 'less'], less: ['css', 'scss', 'less'],
};
...
function walkAndSearch(dir, re, maxResults, context, typeFilter, outputLines, totalMatches) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      walkAndSearch(fullPath, re, maxResults, context, typeFilter, outputLines, totalMatches);
      ...
      for (let i = 0; i < lines.length && outputLines.length < maxResults; i++) {
        const line = lines[i];
        ...
        const end = context > 0 ? Math.min(lines.length, lineNum + context) : lineNum;
...
function grepNative(pattern, searchDir, maxResults, context, typeFilter) {
  ...
  walkAndSearch(searchDir, re, maxResults, context, typeFilter, outputLines, totalMatches);
...
function parseArgs(input) {
  ...
  const maxResults = typeof json.max_results === 'number' && json.max_results >= 1
    ? json.max_results
    : DEFAULT_GREP_MAX_RESULTS;
  const context = typeof json.context === 'number' && json.context >= 0
    ? json.context
    : 0;
    ...
    return toolDef(
      ...
      'Search file contents for a pattern. Supports regex, file type filtering, and context lines. Returns matching lines with file paths.',
      {
        ...
        properties: {
          ...
          path: param('string', 'File or directory to search (default: workspace root)'),
          type: param('string', 'File type filter (e.g., rust, ts, py, js, all)'),
          max_results: param('integer', `Maximum results to return (default: ${DEFAULT_GREP_MAX_RESULTS})`, { minimum: 1 }),
          context: param('integer', 'Number of context lines before/after match (default: 0)'),
        },
        required: ['pattern'],
      }
    );
