...
import { DEFAULT_MAX_TOOL_OUTPUT_LINES, DEFAULT_READ_TOOL_LIMIT } from '../config.js';
...
export class ProjectInfoTool {
  ...
  toToolDef() {
    return toolDef(
      ...
      {
        properties: {
          path: param('string', 'Path to the directory to analyze. Defaults to current directory.'),
          max_depth: param('integer', 'Max directory depth for du. Default: 1.'),
          max_files: param('integer', 'Max files to list. Default: 100.'),
        },
        required: [],
      }
    );
  ...
  async execute(input, ctx) {
    ...
    if (dirSizes.length > 0) {
      ...
      for (const [size, dirPath] of dirSizes) {
        const displayPath = dirPath === '.' ? '.' : dirPath.replace(/^\.\//, '');
  ...
  async _getLastCommitTime(cwd) {
    try {
      const { stdout } = await execFileAsync('git', ['log', '-1', '--format=%ar'], {
        cwd,
        maxBuffer: 1024 * 1024,
      });
      return stdout.trim();
...
function extensionToLanguage(ext) {
  const map = {
    ...
    ts: 'TypeScript', tsx: 'TypeScript', 'd.ts': 'TypeScript', go: 'Go',
    java: 'Java', cpp: 'C++', cc: 'C++', cxx: 'C++', c: 'C',
    h: 'C/C++ Header', rb: 'Ruby', php: 'PHP', swift: 'Swift',
    kt: 'Kotlin', kts: 'Kotlin', scala: 'Scala', ex: 'Elixir', exs: 'Elixir',
    erl: 'Erlang', hrl: 'Erlang', hs: 'Haskell', toml: 'TOML',
    yaml: 'YAML', yml: 'YAML', json: 'JSON', xml: 'XML',
    html: 'HTML', htm: 'HTML', css: 'CSS', scss: 'CSS', sass: 'CSS',
    md: 'Markdown', mdx: 'Markdown', sh: 'Shell', dockerfile: 'Dockerfile',
    tf: 'HCL', proto: 'Protobuf', sql: 'SQL', env: 'Env',
  };
