...
describe("Agent constructor", () => {
  ...
  it("accepts custom options", () => {
    const mockClient = {
      chat: async () => ({ type: "content", content: "test" }),
    };
...
describe("Agent.filteredToolDefs", () => {
  it("returns all defs when no active skills", () => {
    ...
    registry.register("bash", {
      toToolDef: () => ({ function: { name: "bash" } }),
    });
    registry.register("write", {
      toToolDef: () => ({ function: { name: "write" } }),
    });
  ...
  it("filters tools by active skill patterns using non-core tools", () => {
    ...
    registry.register("custom_tool", {
      toToolDef: () => ({ function: { name: "custom_tool" } }),
    });
    registry.register("other_tool", {
      toToolDef: () => ({ function: { name: "other_tool" } }),
    });
    const defs = agent.filteredToolDefs(registry);
...
describe("Agent.executePrompt", () => {
  ...
  it("returns error on unknown prompt", () => {
    const mockLoader = {
      getPrompt: () => null,
      allPrompts: () => [
        { name: "existing-prompt", disableModelInvocation: false },
      ],
    };
...
describe("Agent.regenerateSystemPrompt", () => {
  ...
  it("replaces system message with new content", () => {
    ...
    expect(agent.context.getMessages()[0].content).toContain(
      "# Role & Mission",
    );
  ...
  it("prunes old skill content messages from conversation", () => {
    ...
    const skillMsgsBefore = agent.context
      .messages()
      .filter((m) => m.content && m.content.includes("skill_content")).length;
    agent.regenerateSystemPrompt();
    const skillMsgsAfter = agent.context
      .messages()
      .filter((m) => m.content && m.content.includes("skill_content")).length;
  ...
  it("keeps user messages and assistant responses", () => {
    ...
    const assistantMsgsBefore = agent.context
      .messages()
    ...
    const userMsgsAfter = agent.context
      .messages()
      .filter((m) => m.role === "user").length;
    const assistantMsgsAfter = agent.context
      .messages()
      .filter((m) => m.role === "assistant").length;
...
describe("Agent.handleToolCalls", async () => {
  ...
  it("handles tool not allowed by skills with unknown tool", async () => {
    ...
    const toolCalls = [
      { function: { name: "custom_tool_xyz", arguments: "{}" }, id: "call-1" },
    ];
    await agent.handleToolCalls(toolCalls, registry);
...
describe("Agent.activateSkill", () => {
  ...
  it("adds wrapped skill content to context", () => {
    ...
    const messages = agent.context.getMessages();
    const skillMsg = [...messages]
      .reverse()
      .find((m) => m.role === "user" && m.content.includes("skill_content"));
  ...
  it("includes additionalFiles as skill_resources", () => {
    ...
    const skillMsg = [...messages]
      .reverse()
      .find((m) => m.role === "user" && m.content.includes("skill_resources"));
    expect(skillMsg).toBeDefined();
...
describe("Agent.drainPendingTaskMessages", () => {
  ...
  it("drains messages into context and emits events", () => {
    ...
    agent._pendingTaskMessages = ["Task 1 completed", "Task 2 completed"];
...
describe("Agent.autoActivateSkills", () => {
  ...
  it("calls loader setAvailableTools with tool names", () => {
...
describe("Agent.filteredToolDefs", () => {
  ...
  it("filters tool defs by active skill patterns", () => {
    const agent = new Agent({
      skills: [
        { name: "skill-a", allowedTools: ["read"], includeTools: ["grep"] },
      ],
    });
    agent.activeSkills.add("skill-a");
    const registry = new ToolRegistry();
    ...
    registry.register("read", {
      toToolDef: () => ({ function: { name: "read" } }),
    });
    registry.register("grep", {
      toToolDef: () => ({ function: { name: "grep" } }),
    });
    const defs = agent.filteredToolDefs(registry);
    expect(defs).toHaveLength(2);
