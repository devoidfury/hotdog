...
describe("OutputSink", () => {
  ...
  it("dispatches events to correct handler", () => {
    ...
    sink.emit(
      outputEvent(OUTPUT_EVENT.ASSISTANT_MESSAGE, { content: "hello" }),
    );
    ...
    sink.emit(
      outputEvent(OUTPUT_EVENT.STREAMING_REASONING_CHUNK, { content: "think" }),
    );
  ...
  it("emitToolCall does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitToolCall({ toolName: "bash", input: "{}" }),
    ).not.toThrow();
  });
  ...
  it("emitToolResult does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitToolResult({ toolName: "bash", result: "output" }),
    ).not.toThrow();
  });
  ...
  it("emitCompacting does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitCompacting({ messageCount: 10, keepRecent: 2 }),
    ).not.toThrow();
  });
  ...
  it("emitQuestion does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitQuestion({ question: "What is your name?" }),
    ).not.toThrow();
  });
  ...
  it("emitTaskProgress does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitTaskProgress({ status: "task_result_received" }),
    ).not.toThrow();
  });
  ...
  it("emitUserMessage does nothing by default", () => {
    ...
    // Should not throw
    expect(() =>
      sink.emitUserMessage({ content: "user message" }),
    ).not.toThrow();
  });
...
describe("EVENT_HANDLERS mapping", () => {
  it("maps all event types to handler methods", async () => {
    ...
    expect(EVENT_HANDLERS[OUTPUT_EVENT.ASSISTANT_MESSAGE]).toBe(
    ...
    expect(EVENT_HANDLERS[OUTPUT_EVENT.COMMAND_RESULT]).toBe(
      "emitCommandResult",
    );
    expect(EVENT_HANDLERS[OUTPUT_EVENT.QUESTION]).toBe("emitQuestion");
    expect(EVENT_HANDLERS[OUTPUT_EVENT.STREAMING_CHUNK]).toBe(
      "emitStreamingChunk",
    );
    expect(EVENT_HANDLERS[OUTPUT_EVENT.STREAMING_REASONING_CHUNK]).toBe(
      "emitStreamingReasoningChunk",
    );
