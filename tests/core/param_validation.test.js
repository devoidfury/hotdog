...
import { validateParams, formatValidationErrors } from '../../src/core/param-validation.js';
...
describe('validateParams — required fields', () => {
  it('detects missing required field', () => {
    const result = validateParams(
      {},
      schema({ name: { type: 'string' }, age: { type: 'integer' } }, ['name', 'age'])
    );
    expect(result.valid).toBe(false);
...
describe('validateParams — additionalProperties', () => {
  ...
  it('allows extra fields if they are in required list', () => {
    ...
    const result = validateParams(
      ...
      { type: 'object', properties: { name: { type: 'string' } }, required: ['required_field'], additionalProperties: false }
    );
    expect(result.valid).toBe(true);
  });
});
...
describe('Integration — validation blocks invalid tool execution', () => {
  it('returns error message instead of executing when args are invalid', () => {
    ...
    expect(result2.errors.some(e => e.includes('expected integer'))).toBe(true);
    ...
    expect(result3.errors.some(e => e.includes('exceeds maximum'))).toBe(true);
...
describe('ToolRegistry.validateToolArgs', () => {
  ...
  it('returns null when tool has no parameters', async () => {
    ...
    registry.register('mytool', {
      toToolDef() {
        return { type: 'function', function: { name: 'mytool', description: '' } };
      },
      execute: async () => 'ok',
    });
  ...
  it('passes valid args via registry', async () => {
    ...
    const error = registry.validateToolArgs('testtool', '{"name": "test", "count": 5}');
  ...
  it('handles JSON string input', async () => {
    ...
    // Valid JSON string
    expect(registry.validateToolArgs('testtool', '{"name": "test"}')).toBeNull();
    // Invalid JSON string
    expect(registry.validateToolArgs('testtool', '{"name": 123}')).not.toBeNull();
  });
