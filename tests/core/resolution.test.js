...
describe("resolveTheme", () => {
  ...
  it("handles file path theme", () => {
    expect(resolveTheme("/path/to/theme.json", null)).toBe(
      "/path/to/theme.json",
    );
  });
});
...
describe("resolveModelWithProvider", () => {
  it("returns name as-is when it already contains /", () => {
    expect(resolveModelWithProvider("provider/model", null)).toBe(
      "provider/model",
    );
  });
...
describe("allProfilesForSwitch", () => {
  it("merges config and file profiles", () => {
    const result = allProfilesForSwitch({
      ...
      configProfiles: {
        default: { role: "config-role" },
        minimal: { role: "config-role", aspects: ["aspect-b"] },
      },
