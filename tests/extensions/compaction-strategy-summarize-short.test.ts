// Tests for SummarizeShortStrategy compaction strategy.
...
describe("SummarizeShortStrategy", () => {
  let strategy: SummarizeShortStrategy;

  beforeEach(() => {
    strategy = new SummarizeShortStrategy();
  });

  it("has correct name and description", () => {
    expect(strategy.name).toBe("summarize-short");
    expect(strategy.description).toContain("Aggressive LLM summarization");
  });
  ...
  describe("execute", () => {
    it("returns null when nothing to compact (not enough messages)", async () => {
      const messages = [msg("user", "hello"), msg("assistant", "hi")];
    ...
    it("calls llmChat with short user prompt containing conversation", async () => {
      const messages = [
      ...
      expect(capturedUserPrompt).toContain("CONCISE structured summary");
    ...
    it("skips system messages when counting recent messages to keep", async () => {
      ...
      // System messages are skipped when counting, so we should compact the old messages
      expect(result).not.toBeNull();
      expect(result!.messagesCompacted).toBeGreaterThan(0);
    });
  });
